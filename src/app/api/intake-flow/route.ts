...
export async function GET(req: NextRequest) {
  ...
  try {
    return NextResponse.json(
      await handleIntakeFlow(paramFromReq(req, null, 'companyId'), paramFromReq(req, null, 'askWhen'))
    );
  } catch (error) {
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      await handleIntakeFlow(paramFromReq(req, body, 'companyId'), paramFromReq(req, body, 'askWhen'))
    );
  } catch (error) {
    return jsonError(error, 500);
  }
